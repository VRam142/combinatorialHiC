...
python analyze_scDHC_V2design.py $bc1 /tmp/$fq_r1.split /tmp/$fq_r2.split /tmp/$fq_r1.bc_clipped /tmp/$fq_r2.bc_clipped > $outdir/$bc_assoc
#Align reads to combo-reference using bowtie2
bowtie2 -x combo_hg19_mm10 -p 4 -U /tmp/$fq_r1.bc_clipped 2> $outdir/$fq_r1.mapping_stats | samtools view -bS - > /tmp/$fq_r1.bam&
...
bedtools closest -t first -d -a $fq_r1.bed.mapq0.sorted -b combo_hg19_mm10.dpnii.bed > $fq_r1.bed.mapq0.dre&
bedtools closest -t first -d -a $fq_r2.bed.mapq0.sorted -b combo_hg19_mm10.dpnii.bed > $fq_r2.bed.mapq0.dre&
wait
...
python catbed2bedpe.py $bc_assoc.concat.sorted.bed $bc_assoc $bc_assoc.unmatched.bedpe $bc_assoc.matching.html > $bc_assoc.bedpe.mapq0
...
python dedupe_scDHC.py $bc_assoc.bedpe.mapq0.sorted > $bc_assoc.bedpe.mapq0.deduped
#Check some QC stats
...
python filter_bedpe.py combo_hg19_mm10.genomesize $bc_assoc.bedpe.mapq0.deduped > $bc_assoc.bedpe.mapq0.deduped.filtered
...
python calculate_cell_distro_long.py $bc_assoc.bedpe.mapq0 > $bc_assoc.percentages 2> $bc_assoc.REoccurrences &
...
python incorporate_cistrans_genotype.py $bc_assoc.cistrans.txt $bc_assoc.deduped.percentages > $bc_assoc.deduped.percentages.filterable
#Generate single-cell matrices
python bin_schic.py /net/shendure/vol8/projects/HiC.TCC.DHC.project/nobackup/HiC_resources/combo_hg19_mm10.genomesize $bc_assoc.deduped.percentages.filterable $bedpe.mapq0.deduped.filtered > foo
rm foo
