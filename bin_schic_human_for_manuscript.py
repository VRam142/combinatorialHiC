...
def cell_sort(percentages):
    '''Iterate through the percentages file generated by the scHi-C pipeline, and return a list
        of cell barcodes where the cell has >3000 unique reads and is 0.95 one species'''
    cells = {}
    for line in percentages:
		...
		if float(gtype) > 0.57 and float(gtype) <0.99: continue
                identity = ("human", huc)
                barcode = "%s-%s" % (bc1, bc2)
...
def main():
    ...
    resolutions = [500000,10000000,500000000]
#    resolutions = [500000]
